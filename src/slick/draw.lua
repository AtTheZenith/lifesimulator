...
local function _drawText(world)
  ...
  for _, item in ipairs(items) do
    ...
    for _, shape in ipairs(entity.shapes.shapes) do
      love.graphics.print(
        string.format('%.2f, %.2f', shape.bounds.topLeft.x, shape.bounds.topLeft.y),
        shape.vertices[1].x,
        shape.vertices[1].y
      )
      love.graphics.print(
        string.format('%.2f x %.2f', shape.bounds:width(), shape.bounds:height()),
        shape.vertices[1].x,
        shape.vertices[1].y + 8
      )
    end
  end
end
      ...
      for i = 1, shape.vertexCount do
        ...
        if i <= shape.normalCount then
          ...
          love.graphics.line(
            (a.x + b.x) / 2,
            (a.y + b.y) / 2,
            (a.x + b.x) / 2 + n.x * localSize,
            (a.y + b.y) / 2 + n.y * localSize
          )
        end
      end
    end
