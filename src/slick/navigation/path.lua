...
function path:_funnel()
  ...
  while index <= n do
    ...
    if slickmath.direction(right.point, otherRight.point, apex.point, slickmath.EPSILON) <= 0 then
      if
        apex.index == right.index
        or slickmath.direction(left.point, otherRight.point, apex.point, slickmath.EPSILON) > 0
      then
        right = otherRight
        rightIndex = index
      else
    ...
    if not skip and slickmath.direction(left.point, otherLeft.point, apex.point, slickmath.EPSILON) >= 0 then
      if
        apex.index == left.index
        or slickmath.direction(right.point, otherLeft.point, apex.point, slickmath.EPSILON) < 0
      then
        left = otherLeft
        leftIndex = index
      else
