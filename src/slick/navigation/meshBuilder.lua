...
function navMeshBuilder:_prepareLayers(options)
  for _, layer in ipairs(self.layers) do
    ...
    if #meshes >= 1 then
      local currentPoints, currentEdges, currentUserdata, currentExteriorEdges, currentInteriorEdges =
        meshes[1].inputPoints,
        meshes[1].edges,
        meshes[1].inputUserdata,
        meshes[1].inputExteriorEdges,
        meshes[1].inputInteriorEdges

      for i = 2, #meshes do
        currentPoints, currentEdges, currentUserdata, currentExteriorEdges, currentInteriorEdges = self.clipper:clip(
          clipper.union,
          currentPoints,
          { currentExteriorEdges, currentInteriorEdges },
          meshes[i].inputPoints,
          { meshes[i].inputExteriorEdges, meshes[i].inputInteriorEdges },
          options,
          currentUserdata,
...
function navMeshBuilder:_combineLayers(options)
  local currentPoints, currentEdges, currentUserdata, currentExteriorEdges, currentInteriorEdges =
    self.layers[1].mesh.inputPoints,
    self.layers[1].mesh.edges,
    self.layers[1].mesh.inputUserdata,
    self.layers[1].mesh.inputExteriorEdges,
  ...
  for i = 2, #self.layers do
    ...
    if func and layer.mesh then
      currentPoints, currentEdges, currentUserdata, currentExteriorEdges, currentInteriorEdges = self.clipper:clip(
        ...
        currentPoints,
        { currentExteriorEdges, currentInteriorEdges },
        layer.mesh.inputPoints,
        { layer.mesh.inputExteriorEdges, layer.mesh.inputInteriorEdges },
        options,
        currentUserdata,
  ...
  if #self.layers == 1 then
    currentPoints, currentEdges, currentUserdata =
      self.cache.triangulator:clean(currentPoints, currentEdges, currentUserdata, options)
  end
