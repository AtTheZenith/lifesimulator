...
function clipper:_intersect(intersection)
  ...
  userdata.isExteriorEdge = userdata.isExteriorEdge
    or intersection.a1Userdata.isExteriorEdge
    or intersection.a2Userdata.isExteriorEdge
    or intersection.b1Userdata.isExteriorEdge
    or intersection.b2Userdata.isExteriorEdge
  userdata.isInteriorEdge = userdata.isInteriorEdge
    or intersection.a1Userdata.isInteriorEdge
    or intersection.a2Userdata.isInteriorEdge
    or intersection.b1Userdata.isInteriorEdge
    or intersection.b2Userdata.isInteriorEdge
...
function clipper:_addPolygon(points, exterior, interior, userdata, polygon)
  ...
  local _, triangleCount, _, polygonCount = self.triangulator:triangulate(
    polygon.points,
    polygon.edges,
    _triangulateOptions,
    polygon.triangles,
  ...
  for i = 1, #self.resultPolygon.points, 2 do
    ...
    if not combinedUserdata.hasEdge then
      ...
      elseif
        ...
        and (self:_pointInside(x, y, self.subjectPolygon) and self:_pointInside(x, y, self.otherPolygon))
      then
        self:_addResultEdge(index)
      end
    end
...
function clipper:_segmentInsidePolygon(s, polygon, vertices)
  ...
  for i = 1, #vertices do
    ...
    isABCollinear = isABCollinear
      or slickmath.collinear(self.cachedSegment.a, self.cachedSegment.b, s.a, s.b, self.triangulator.epsilon)

    local intersection, _, _, u, v =
      slickmath.intersection(self.cachedSegment.a, self.cachedSegment.b, s.a, s.b, self.triangulator.epsilon)
    if
      intersection
      and u
      and v
      and (u > self.triangulator.epsilon and u + self.triangulator.epsilon < 1)
      and (v > self.triangulator.epsilon and v + self.triangulator.epsilon < 1)
    then
...
function clipper:_pointInsidePolygon(p, polygon, vertices)
  ...
  for i = 1, #vertices do
    ...
    isCollinear = isCollinear
      or slickmath.collinear(self.cachedSegment.a, self.cachedSegment.b, p, p, self.triangulator.epsilon)
...
function clipper:_addResultEdge(a, b)
  ...
  if a and b then
    ...
    if
      self.resultExteriorEdges
      and (self.resultPolygon.userdata[a].isExteriorEdge or self.resultPolygon.userdata[b].isExteriorEdge)
    then
    ...
    if
      self.resultInteriorEdges
      and (self.resultPolygon.userdata[a].isInteriorEdge or self.resultPolygon.userdata[b].isInteriorEdge)
    then
...
function clipper:intersection(a, b)
  ...
  local hasAnyCollinearOtherPoints =
    self:_hasAnyOnSide(ax, ay, bx, by, 0, self.otherPolygon, aOtherPolygons, bOtherPolygons)
  local hasAnyCollinearSubjectPoints =
    self:_hasAnyOnSide(ax, ay, bx, by, 0, self.otherPolygon, aOtherPolygons, bOtherPolygons)

  if
    (abInsideOther and abInsideSubject)
    or (
      not abCollinearOther
      and ((abInsideOther and hasAnyCollinearSubjectPoints) or (abInsideSubject and hasAnyCollinearOtherPoints))
    )
  then
...
function clipper:clip(
  ...
  self.triangulator:clean(
    self.combinedPoints,
    self.combinedEdges,
    self.combinedUserdata,
    self.clipCleanupOptions,
    self.resultPolygon.points,
    self.resultPolygon.edges,
    self.resultPolygon.userdata
  )
