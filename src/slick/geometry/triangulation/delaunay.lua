  ...
  while index <= #sortedPoints do
    ...
    while nextIndex <= #sortedPoints and sortedPoint.point:distance(sortedPoints[nextIndex].point) < self.epsilon do
      ...
      self.dissolve:init(
        ...
        nextPoint.id,
        userdata and userdata[nextPoint.id]
      )
...
function delaunay:_splitEdgesAgainstEdges(intersect, userdata)
  ...
  for i = 2, #sortedEdges do
    ...
    for j, otherEdge in ipairs(activeEdges) do
      ...
      local connected = (
        selfEdge.edge.a == otherEdge.edge.a
        or selfEdge.edge.a == otherEdge.edge.b
        or selfEdge.edge.b == otherEdge.edge.a
        or selfEdge.edge.b == otherEdge.edge.b
      )
...
function delaunay:clean(points, edges, userdata, options, outPoints, outEdges, outUserdata)
  ...
  for i = 1, #points, 2 do
    ...
    table.insert(
      self.sortedPoints,
      search.lessThan(self.sortedPoints, sortedPoint, delaunaySortedPoint.compare) + 1,
      sortedPoint
    )
  end
...
function delaunay:triangulate(points, edges, options, result, polygons)
  ...
  local polygonization = options.polygonization == nil and defaultTriangulationOptions.polygonization
    or options.polygonization
  ...
  if self.debug then
    assert(
      points and #points >= 6 and #points % 2 == 0,
      'expected three or more points in the form of x1, y1, x2, y2, ..., xn, yn'
    )
    assert(
      not edges or #edges == 0 or #edges % 2 == 0,
...
function delaunay:_testFlipTriangle(a, b, x)
  ...
  local result = slickmath.inside(self.points[a], self.points[b], self.points[x], self.points[y])
  if result < 0 then
  ...
  for i = 1, #self.points do
    ...
    for j = 2, #vertices, 2 do
      ...
      if second < first and not self:_isTriangleEdgeConstrained(first, second) then
        ...
        if y then
          local result = slickmath.inside(self.points[first], self.points[second], self.points[x], self.points[y])
...
function delaunay:_mergePolygons(
  destinationPolygon,
  sourcePolygon,
  destinationPolygonVertexIndex,
  sourcePolygonVertexIndex
)
