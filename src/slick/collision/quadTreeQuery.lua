...
function quadTreeQuery:_performPointQuery(node, p, E)
  ...
  if
    slickmath.withinRange(p.x, node:left(), node:right(), E)
    and slickmath.withinRange(p.y, node:top(), node:bottom(), E)
  then
    if #node.children > 0 then
      ...
      for _, d in ipairs(node.data) do
        ...
        if
          not self.data[d]
          and slickmath.withinRange(p.x, r:left(), r:right(), E)
          and slickmath.withinRange(p.y, r:top(), r:bottom(), E)
        then
...
function quadTreeQuery:_performSegmentQuery(node, s, E)
  ...
  local overlaps = (s:left() <= node:right() + E and s:right() + E >= node:left())
    and (s:top() <= node:bottom() + E and s:bottom() + E >= node:top())
  if overlaps then
    if #node.children > 0 then
      for _, c in ipairs(node.children) do
