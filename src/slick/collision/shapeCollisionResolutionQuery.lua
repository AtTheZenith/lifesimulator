...
function shapeCollisionResolutionQuery:_isShapeMovingAwayFromShape(a, b, aOffset, bOffset, scalar)
  ...
  for i = 1, otherVertexCount do
    ...
    for k = 1, currentVertexCount do
      ...
      local direction =
        slickmath.direction(_cachedOtherSegment.a, _cachedOtherSegment.b, _cachedCurrentPoint, self.epsilon)
      if direction < 0 then
        sameSide = false
        break
      end
...
function shapeCollisionResolutionQuery:_performPolygonPolygonProjection(
  selfShape,
  otherShape,
  selfOffset,
  otherOffset,
  ...
  if hit and (self.depth == math.huge or self.depth < self.epsilon) and _cachedRelativeVelocity:lengthSquared() > 0 then
    hit = not (
      self:_isShapeMovingAwayFromShape(self.currentShape, self.otherShape, selfOffset, otherOffset, 1)
      or self:_isShapeMovingAwayFromShape(self.otherShape, self.currentShape, otherOffset, selfOffset, -1)
    )
  end
  ...
  if hit and self.firstTime < 0 and self.axis.parent == self.currentShape then
    ...
    local otherDirection = slickmath.direction(
      self.otherAxis.segment.a,
      self.otherAxis.segment.b,
      self.currentShape.shape.center,
      self.epsilon
    )
  ...
  if side == SIDE_RIGHT or side == SIDE_LEFT then
    ...
    if _cachedSegmentA:overlap(_cachedSegmentB) then
      intersection, x, y =
        slickmath.intersection(_cachedSegmentA.a, _cachedSegmentA.b, _cachedSegmentB.a, _cachedSegmentB.b, self.epsilon)

      if intersection and not (x and y) then
        intersection = slickmath.intersection(
          _cachedSegmentA.a,
          _cachedSegmentA.a,
          _cachedSegmentB.a,
          _cachedSegmentB.b,
  ...
  elseif side == SIDE_NONE then
    for j = 1, selfShape.vertexCount do
      ...
      for k = 1, otherShape.vertexCount do
        ...
        if _cachedSegmentA:overlap(_cachedSegmentB) then
          local intersection, x, y = slickmath.intersection(
            _cachedSegmentA.a,
            _cachedSegmentA.b,
            _cachedSegmentB.a,
            _cachedSegmentB.b,
...
--- @param selfVelocity slick.geometry.point
--- @param otherVelocity slick.geometry.point
function shapeCollisionResolutionQuery:performProjection(
  selfShape,
  otherShape,
  selfOffset,
  otherOffset,
